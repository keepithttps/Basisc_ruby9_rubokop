...
class Train
  ...
  def each_wagon_train(wagons, &block)
    wagons.each(&block)
  end
  ...
  def validate!
    ...
    if number.split('').length + 1 != FULL_NUMBER_TRAIN
      raise "Number should be at least #{FULL_NUMBER_TRAIN} symbols = #{number.split('').length + 1} "
    end
